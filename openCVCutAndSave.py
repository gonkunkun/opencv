...
for tclass in classes:
    ...
    success_path = FLAGS.input_dir + tclass + "/success"
    if not os.path.exists(success_path):
        os.mkdir(success_path)
    # fail_path = FLAGS.input_dir + tclass + "/fail"
                ...
                if (FLAGS.resize):
                    try:
                        cropped_img = cv2.resize(
                            cropped_img, (FLAGS.resize, FLAGS.resize))
                    except:
...
print()
try:
    print("Success Rate: " + str(round(face_detect_count/total_image_count*100, 2)
                                 ) + "% (" + str(face_detect_count) + "/" + str(total_image_count) + ")")
except:
